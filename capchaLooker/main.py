...
def pictures_recover(bgImg, slideImg, folder):
    ...
    bgImg = cv2.imread(folder + 'bgImg.jpeg')
    img2 = Image.open(folder + 'slideImg.png')
    xpos = 20
    ypos = 20
    
    region = img2.crop((xpos + 18, ypos + 18, xpos + 76, ypos + 76))
    region.save(folder + 'snapshot2_cut.png')

    imobj_cut = cv2.imread(folder + 'snapshot2_cut.png')
    template = cv2.imread(folder + 'snapshot2_cut.png', 0)
    w, h = template.shape[::-1]

    result = cv2.matchTemplate(bgImg, imobj_cut, cv2.TM_CCORR_NORMED)
    ...
    plt.savefig(folder + 'match_result.png')
    # plt.show()
